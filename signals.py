# signals.py — With 200 EMA Trend Filter
...
def generate_signal(symbol):
    ...
    if buy_votes > sell_votes and buy_votes >= config.MIN_VOTES:
        action = 'BUY'
        confidence = 'HIGH' if buy_votes >= 5 else 'MEDIUM'
    elif sell_votes > buy_votes and sell_votes >= config.MIN_VOTES:
        action = 'SELL'
        confidence = 'HIGH' if sell_votes >= 5 else 'MEDIUM'
    else:
        action = 'HOLD'
        confidence = 'LOW'
    ...
    if action == 'BUY' and price < ema_200:
        print(f"  [FILTER] {symbol} BUY skipped — price below 200 EMA")
        action = 'HOLD'
        confidence = 'LOW'
    elif action == 'SELL' and price > ema_200:
        print(f"  [FILTER] {symbol} SELL skipped — price above 200 EMA")
        action = 'HOLD'
        confidence = 'LOW'
